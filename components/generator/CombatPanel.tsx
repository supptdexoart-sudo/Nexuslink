...
import { Crown, Heart, Swords, Shield, Zap, X, Skull, Dice5, Wind } from 'lucide-react';
...
const CombatPanel: React.FC<CombatPanelProps> = ({ event, onUpdate }) => {
    ...
    // Helper to calculate flee chance based on rarity
            ...
            <div className="flex flex-col sm:flex-row gap-4 text-xs bg-black/40 p-3 rounded border border-zinc-800/50">
                <div className="flex-1">
                    ...
                    <ul className="text-zinc-600 space-y-1 list-disc pl-3">
                        <li>Hráč hází kostkou na útok.</li>
                        <li>Poškození = Hod - <strong>Obrana</strong>.</li>
                        <li>Pokud Poškození {'>'} 0, odečte se od <strong>HP</strong>.</li>
                    </ul>
                </div>
                {!isTrap && (
                    <div className="flex-1 border-l border-zinc-800 pl-4">
                        ...
                        <p className="text-[10px] text-zinc-600 mt-1">Legendární monstra se loví, před těmi se neutíká.</p>
                    </div>
                )}
            </div>
        </div>
