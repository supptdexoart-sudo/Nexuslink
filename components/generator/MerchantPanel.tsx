...
import { ShoppingBag, Coins, Plus, Trash2, Swords, Cross, Wand2, Footprints } from 'lucide-react';
    ...
    return (
        <div className="bg-gradient-to-br from-yellow-900/30 to-black border border-yellow-700 rounded-xl p-5 shadow-[0_0_30px_rgba(234,179,8,0.1)]">
            ...
            <div className="flex gap-2 mb-4 flex-wrap">
                <input
                    type="text"
                    placeholder="ID Předmětu (např. ITEM-001)"
                    value={merchantItemId}
                    onChange={(e) => setMerchantItemId(e.target.value)}
                    className="flex-[2] bg-zinc-900 border border-zinc-700 rounded p-2 text-white font-mono text-sm min-w-[150px]"
                />
                <input
                    type="number"
                    placeholder="Ks"
                ...
                <div className="flex flex-col gap-1 w-24">
                    <label className="text-[9px] text-zinc-500 uppercase">Cena (Nákup)</label>
                    <input
                        type="number"
                        placeholder="Cena"
                        value={merchantItemPrice}
                        onChange={(e) => setMerchantItemPrice(parseInt(e.target.value))}
                        className="w-full bg-zinc-900 border border-zinc-700 rounded p-2 text-yellow-500 font-mono text-sm font-bold"
                    />
                </div>
                <div className="flex flex-col gap-1 w-24">
                    <label className="text-[9px] text-zinc-500 uppercase">Cena (Výkup)</label>
                    <input
                        type="number"
                        placeholder="Výkup"
                        value={merchantItemSellPrice}
                        onChange={(e) => setMerchantItemSellPrice(parseInt(e.target.value))}
                        className="w-full bg-zinc-900 border border-zinc-700 rounded p-2 text-green-500 font-mono text-sm font-bold"
                    />
                </div>
                <button type="button" onClick={addMerchantItem} className="px-4 bg-yellow-600 hover:bg-yellow-500 text-black font-bold rounded flex items-center justify-center self-end h-10 mt-auto">
            ...
            <div className="space-y-2">
                {event.merchantItems?.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-zinc-800 p-2 rounded border border-zinc-700">
                        <div>
                            <span className="text-white font-mono font-bold">{item.id}</span>
                            <span className="text-zinc-500 text-xs ml-2">x{item.stock}</span>
                            <div className="inline-flex gap-3 ml-3">
                                <span className="text-yellow-500 text-xs font-mono font-bold">Nákup: {item.price || 0}</span>
                                <span className="text-green-500 text-xs font-mono font-bold">Prodej: {item.sellPrice || 0}</span>
                            </div>
                        </div>
                        <button type="button" onClick={() => removeMerchantItem(idx)} className="text-red-500 hover:text-white"><Trash2 className="w-4 h-4" /></button>
                    </div>
                ))}
                {(!event.merchantItems || event.merchantItems.length === 0) && (
                    <p className="text-zinc-600 text-xs italic text-center">Žádné zboží na skladě.</p>
                )}
            </div>
        </div>
    );
