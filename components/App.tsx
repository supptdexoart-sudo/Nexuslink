...
const inventoryImport = () => import('./InventoryView');
const generatorImport = () => import('./Generator');
const roomImport = () => import('./Room');
const settingsImport = () => import('./SettingsView');
const spaceshipImport = () => import('./SpaceshipView');
              ...
              {logic.activeTab === Tab.INVENTORY && (
                <motion.div key="inventory" {...({ initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } } as any)} className="absolute inset-0">
                  <InventoryView 
                    ...
                    onEditItem={logic.handleEditInventoryItem} // Pass edit handler
                  />
                </motion.div>
              )}
              ...
              {logic.activeTab === Tab.GENERATOR && (
                <motion.div key="generator" {...({ initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } } as any)} className="absolute inset-0">
                  <Generator 
                    ...
                    onSaveCard={(e: GameEvent) => logic.handleSaveEvent(e, true)} 
              ...
              {logic.activeTab === Tab.SPACESHIP && (
                <motion.div key="spaceship" {...({ initial: { opacity: 0 }, animate: { opacity: 1, exit: { opacity: 0 } } } as any)} className="absolute inset-0">
                  <SpaceshipView 
                    ...
                    onAnalyzePlanet={logic.handleAnalyzePlanet} 
                    onResetPlanet={logic.handleResetPlanetProgress} // Passed down
                    isAdmin={logic.isAdmin} // Passed
                    isTestMode={logic.isTestMode} // Passed
                  />
                </motion.div>
              )}
            </AnimatePresence>
