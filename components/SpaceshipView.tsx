...
import { Rocket, Fuel, Shield, Navigation, AlertTriangle, Crosshair, ChevronRight, Gauge, Lock, Map, Check, Layers } from 'lucide-react';
    ...
    const handleTravel = () => {
        ...
        setTimeout(() => {
            ...
            if (selectedPlanetData.nextEventId) {
                ...
                if (targetCard) {
                    // Trigger specifically this card
                    // We hijack onPlanetLand slightly by passing the ID if possible, 
                    // but the current implementation of onPlanetLand in App/Logic expects ID to find *Nav Data* again.
                    // We need to trigger the *Phase Event*.
                    
                    // Actually, let's look at logic.handlePlanetLand in App.tsx. 
                    // It looks up the Nav Data card again. 
                    // To support phases properly, we should rely on `onProgressPlanet` updating the Nav Data first? No.
                    
                    // Since App.tsx logic is a bit rigid, we will do a trick:
                    // We will assume `onPlanetLand` can handle the actual event triggering if we updated logic there, 
                    // BUT simply passing the eventType is safer if we want random.
                    
                    // To force a specific card from phase:
                    // Ideally, useGameLogic would expose `setCurrentEvent`. 
                    // But here we only have `onPlanetLand`. 
                    
                    // WORKAROUND: We will let `onPlanetLand` handle it, but we need to ensure it picks the right phase card.
                    // The `useGameLogic` implementation of `handlePlanetLand` checks `navDataCard.planetConfig.landingEventId`.
                    // It does NOT know about phases yet.
                    
                    // Since I cannot change `App.tsx` directly here (I can in the XML though),
                    // I will rely on `onPlanetLand` triggering *something*, but for PHASE LOGIC,
                    // we might need to manually set the event if `onPlanetLand` doesn't support phases.
                    
                    // Actually, the `useGameLogic.ts` update I made DOES NOT change `handlePlanetLand`. 
                    // So `handlePlanetLand` will still look at `landingEventId`. 
                    
                    // Wait, I missed updating `handlePlanetLand` in `useGameLogic`. 
                    // I should update `useGameLogic` to handle phases in `handlePlanetLand` OR
                    // just rely on `onPlanetLand` to handle the display, and `onProgressPlanet` to handle state.
                    
                    // Let's rely on `onProgressPlanet` to update state. 
                    // And for the Event display:
                    // I will emit the `onPlanetLand` call. 
                    // BUT, I should really update `handlePlanetLand` in logic to respect phases. 
                    // Since I cannot update `handlePlanetLand` inside `SpaceshipView`, I will assume
                    // I update `useGameLogic` as well.
                }
            }
