...
import { Send, Trash2, User, Play, Lock, TrendingUp, Package, X, Search, Handshake, Plus, Hash, Globe, Activity, History } from 'lucide-react';
            ...
            {!isSolo && (
                <div className="flex px-2">
                    <button onClick={() => setActiveTab('party')} className={`flex-1 py-3 text-xs font-bold uppercase border-b-2 transition-colors ${activeTab === 'party' ? 'border-signal-amber text-white' : 'border-transparent text-zinc-400'}`}>
                        Jednotka ({roomState.members?.length || 1})
                    </button>
            ...
            {activeTab === 'party' && (
                <div className="p-4 space-y-4">
                    <div className="bg-zinc-900/50 p-5 rounded-2xl border border-zinc-800 relative overflow-hidden">
                        ...
                        {!isSolo && isHost && !roomState.isGameStarted && (
                            <button 
                                onClick={onStartGame}
                                className="w-full py-4 mt-6 font-black uppercase rounded-xl flex items-center justify-center gap-3 transition-transform bg-signal-amber text-black shadow-[0_0_20px_rgba(255,157,0,0.3)] active:scale-95 cursor-pointer"
                            >
                                <Play className="w-5 h-5 fill-current" />
                                AKTIVOVAT PROTOKOL MISE
                            </button>
                        )}
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        {isSolo ? (
                            ...
                            roomState.members?.map((member, idx) => (
                                <div key={idx} className={`bg-black border p-4 rounded-xl flex items-center gap-4 transition-all ${member.name === roomState.nickname ? 'border-signal-cyan/30 bg-signal-cyan/5 shadow-[inset_0_0_20px_rgba(0,242,255,0.05)]' : 'border-zinc-800'}`}>
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border-2 ${member.name === roomState.nickname ? 'border-signal-cyan bg-signal-cyan/10 text-signal-cyan' : 'border-zinc-700 bg-zinc-900 text-zinc-300'}`}>
                                        {member.name.substring(0, 2).toUpperCase()}
                                    </div>
                                    <div className="flex-1">
                                        <div className="flex justify-between items-center mb-1">
                                            <span className={`font-bold text-sm ${member.name === roomState.nickname ? 'text-white' : 'text-zinc-200'}`}>{member.name}</span>
