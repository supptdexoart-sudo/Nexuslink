...
import { MapPin, Skull, Zap, Box, Star, Save, RefreshCw, Trash2, Loader2, AlertTriangle, Activity, Shield, Hash, Share2, CheckCircle, HeartCrack, Heart, Coins, Pencil, Split, HelpCircle, ArrowRight, QrCode, PlayCircle, Ghost, X, Sword, Target, Backpack, BriefcaseMedical, Wind, Lock, Crown, Siren } from 'lucide-react'; 
...
const DilemmaResult: React.FC<DilemmaResultProps> = ({ option, onClose }) => (
    ...
    >
         ...
         <p className="text-zinc-300 mb-8 italic text-lg leading-relaxed">"{option.consequenceText}"</p>
         {option.effectType !== 'none' && (
             <div className={`mb-8 p-6 rounded-2xl border flex items-center justify-center gap-4 ${
                 ...
                 <div className="flex flex-col items-start">
                     <span className="font-mono font-bold text-2xl">{option.effectValue > 0 ? '+' : ''}{option.effectValue} {option.effectType.toUpperCase()}</span>
         ...
         <button onClick={onClose} className="w-full py-4 bg-neon-purple hover:bg-fuchsia-600 text-white font-bold rounded-xl uppercase tracking-widest shadow-[0_0_20px_rgba(188,19,254,0.4)] transition-transform active:scale-95">Pokračovat ve hře</button>
    </motion.div>
);

// --- MAIN COMPONENT ---
const EventCard: React.FC<EventCardProps> = ({ event, onClose, onSave, onDelete, onEdit, onResolveDilemma, onUse, isSaved, isInstantEffect, effectValue, isAdmin, inventory, onPlayerDamage, onStartRaid }) => {
  ...
  const isBoss = event.type === GameEventType.BOSS; // New Check
  ...
  useEffect(() => {
     if(isBoss) {
         playSound('error'); // Ominous sound for boss
  ...
  const getRarityStyles = (rarity: string) => {
    if (event.type === GameEventType.BOSS) {
        return {
            ...
            shadow: 'shadow-[0_0_60px_rgba(153,27,27,0.6)]'
        };
    }
  ...
  // --- COMBAT LOGIC (UNCHANGED FOR STANDARD ENCOUNTERS) ---
  const handleAttack = () => {
  ...
  const handleUseCombatItem = (item: GameEvent) => {
      let used = false;
      if (item.stats) { item.stats.forEach(s => { const label = s.label.toUpperCase(); const val = parseInt(String(s.value)); if (['HP', 'HEAL', 'LÉČENÍ'].some(k => label.includes(k))) { if (onPlayerDamage) onPlayerDamage(val); setCombatLog(prev => [...prev, `> Použit ${item.title}: +${val} HP`]); used = true; } if (['DMG', 'ÚTOK'].some(k => label.includes(k))) { const dmg = Math.abs(val); setEnemyCurrentHp(prev => Math.max(0, prev - dmg)); setCombatLog(prev => [...prev, `> Použit ${item.title}: -${dmg} Enemy HP`]); used = true; } }); }
      if (used) { playSound('heal'); setShowInventory(false); } else { alert("Tento předmět nelze v boji použít (nemá staty)."); }
  };
  ...
  if (isBoss) {
      return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-red-950/95 backdrop-blur-lg">
            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-10"></div>
            
            <motion.div 
                initial={{ scale: 0.9, y: 20 }}
                animate={{ scale: 1, y: 0 }}
                className="relative w-full max-w-sm bg-black border-4 border-red-900 rounded-3xl overflow-hidden shadow-[0_0_100px_rgba(220,38,38,0.5)] flex flex-col"
            >
                {onEdit && (
                    <button onClick={onEdit} className="absolute top-4 left-4 z-20 text-red-500 hover:text-white"><Pencil className="w-5 h-5"/></button>
                )}
  ...
  if (event.type === GameEventType.DILEMA) {
     // ... (Existing Dilemma Render Logic - Kept same as before) ...
     return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
             ...
             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md bg-zinc-950 border-2 border-neon-purple rounded-[30px] overflow-hidden shadow-[0_0_50px_rgba(188,19,254,0.2)] flex flex-col max-h-[90vh] relative">
                {/* Delete Confirm Overlay (Same as before) */}
                {showDeleteConfirm && (
  ...
  if (isEncounter) {
      return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-red-950/80 backdrop-blur-md">
            {/* INVENTORY OVERLAY */}
            <AnimatePresence>
                {showInventory && (
                    <motion.div initial={{y: '100%'}} animate={{y: 0}} exit={{y: '100%'}} className="absolute inset-0 z-[70] bg-zinc-900 flex flex-col">
                        <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-black">
                            <h3 className="font-bold text-white flex items-center gap-2"><Backpack className="w-5 h-5"/> Rychlý Inventář</h3>
                            <button onClick={() => setShowInventory(false)}><X className="w-6 h-6 text-zinc-500"/></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 space-y-3">
                            {inventory?.filter(i => i.isConsumable).map(item => (
                                <button key={item.id} onClick={() => handleUseCombatItem(item)} className="w-full bg-zinc-800 p-3 rounded-lg flex justify-between items-center border border-zinc-700 hover:border-neon-blue">
                                    <span className="font-bold text-white">{item.title}</span>
                                    <span className="text-[10px] bg-zinc-900 px-2 py-1 rounded text-zinc-400">POUŽÍT</span>
                                </button>
                            ))}
                            {(!inventory || inventory.filter(i => i.isConsumable).length === 0) && <p className="text-center text-zinc-500 mt-10">Žádné použitelné předměty.</p>}
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>

            <motion.div className="w-full max-w-sm bg-zinc-950 border-4 border-red-600 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                {/* Header */}
                <div className="bg-red-900/50 p-6 text-center border-b border-red-600 relative">
                    ...
                    {onEdit && (
                        <button onClick={onEdit} className="absolute top-4 left-4 text-red-300 hover:text-white transition-colors">
                ...
                <div className="p-6 space-y-6">
                    ...
                    {/* Combat Log */}
                    <div className="h-24 bg-black rounded-lg p-2 overflow-y-auto font-mono text-xs text-green-500 border border-zinc-800 space-y-1">
                        {combatLog.map((log, i) => (
                            <div key={i} className={log.includes('SELHAL') ? 'text-red-500 font-bold' : ''}>{log}</div>
                        ))}
                    </div>
                </div>
                ...
                <div className="p-4 bg-zinc-900 border-t border-zinc-800 mt-auto">
                    {victory ? (
                        <div className="space-y-3">
                            <div className="bg-green-500/20 border border-green-500 p-3 rounded-lg text-center text-green-500 font-bold uppercase animate-pulse">
                                Vítězství!
                            </div>
                            <button onClick={onClose} className="w-full py-4 bg-zinc-800 text-white font-bold uppercase rounded-xl">Zavřít</button>
                            {/* Potential Loot Button could go here */}
                        </div>
                    ) : (
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                ...
                                <button 
                                    ...
                                    className="flex-1 bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:bg-zinc-800 text-white font-bold uppercase rounded-xl shadow-lg flex items-center justify-center gap-2"
                                >
                            ...
                            <div className="flex gap-2">
                                <button 
                                    ...
                                    className="flex-1 py-3 bg-zinc-800 text-zinc-300 font-bold uppercase rounded-xl text-xs flex items-center justify-center gap-2 border border-zinc-700"
                                >
                                    <BriefcaseMedical className="w-4 h-4" /> Batoh
                                </button>
                                <button 
  ...
  // --- STANDARD ITEM RENDER (EXISTING CODE) ---
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        ...
        <div className={`h-48 ${style.headerBg} flex items-center justify-center relative shadow-inner overflow-hidden shrink-0`}>
           ...
           <motion.div initial={{ scale: 0, rotate: -45 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", delay: 0.2 }} className={`relative z-10 ${style.iconColor} drop-shadow-[0_0_15px_rgba(0,0,0,0.5)]`}>{getIcon()}</motion.div>
        ...
        <div className="flex-1 p-6 flex flex-col bg-zinc-900 overflow-y-auto">
            <div className="flex justify-between items-start mb-4 pb-4 border-b border-zinc-800">
                ...
                <div className="flex flex-col items-end gap-1.5 pl-2"><span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded border ${style.badge}`}>{event.type}</span>{event.price && event.price > 0 && (<span className="text-[10px] font-mono font-bold text-yellow-500 flex items-center gap-1 bg-black/50 px-2 py-1 rounded border border-zinc-800"><Coins className="w-3 h-3" /> {event.price}</span>)}</div>
            </div>
            ...
            <div className="flex gap-2 h-14">
                 {isInstantEffect ? (
                    ...
                    <>
                        ...
                        {isSaved && onDelete && <button onClick={handleDeleteClick} className="w-14 flex items-center justify-center bg-red-900/20 border border-red-900 hover:bg-red-900/40 text-red-500 rounded-xl transition-colors"><Trash2 className="w-5 h-5" /></button>}
                    </>
                 )}
            </div>
        </div>
